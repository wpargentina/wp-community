...
function wp_community_credits() {
	$credits = sprintf( __( 'Copyright %s', 'wp_community' ), '&copy; ' . date( 'Y' ) . ' <a href="' . home_url() . '">' . get_bloginfo( 'name' ) . '</a>. ' ) . sprintf( __( 'Powered by %s', 'wp_community' ), '<a href="http://www.wordpress.org/" class="wp-url">WordPress</a> ' . __( 'and', 'wp_community' ) . ' <a href="http://github.com/andrezrv/follet" class="theme-url">Follet</a>.' );
